...
export function buildCoachPrompt(ctx: CoachContext): {
  ...
  switch (ctx.trigger) {
    ...
    case 'life_event_after':
      user = `The player chose: "${ctx.chosenLabel}" for the event "${ctx.eventTitle}". Portfolio went from CHF ${ctx.portfolioBefore?.toFixed(0)} to CHF ${ctx.portfolioAfter?.toFixed(0)}. ${ctx.socialProofPct ? `${ctx.socialProofPct}% of other players chose the same.` : ''} Give a brief, non-judgmental comment about their choice.`;
      break;

    case 'panic_rebalance':
      user = `The player just shifted heavily toward cash/bonds during a market drawdown of ${ctx.currentDrawdownPct.toFixed(1)}%. They've now got ${ctx.cashPct.toFixed(0)}% in cash. This is their ${ctx.panicRebalances}${ctx.panicRebalances === 1 ? 'st' : ctx.panicRebalances === 2 ? 'nd' : 'th'} panic rebalance. Gently explain the concept of "selling low".`;
      break;

    case 'all_in_equity':
      user = `The player just moved to ${ctx.positions.filter((p) => p.assetId !== 'cash' && p.assetId !== 'ch_bond' && p.assetId !== 'gold_chf').reduce((s, p) => s + p.pct, 0)}% equities. Briefly mention diversification and why having some bonds or gold can smooth the ride.`;
      break;

    case 'cash_heavy':
      user = `The player has had more than 50% in cash for ${ctx.cashHeavyWeeks} weeks (about ${Math.round((ctx.cashHeavyWeeks || 0) / 4)} months). Their total portfolio is CHF ${ctx.totalPortfolio.toFixed(0)}. Briefly explain cash drag and inflation risk.`;
      break;

    case 'milestone':
      user = `The player's portfolio just hit CHF ${ctx.totalPortfolio.toFixed(0)}, up from CHF ${ctx.startingPortfolio.toFixed(0)}. They're in Year ${ctx.currentYear}. Give a brief, encouraging note about compound growth.`;
      break;

    case 'game_end':
      user = `The simulation is over. Final portfolio: CHF ${ctx.totalPortfolio.toFixed(0)} (started with CHF ${ctx.startingPortfolio.toFixed(0)}). They made ${ctx.totalRebalances} rebalances, ${ctx.panicRebalances} during drawdowns. Cash was over 50% for ${ctx.cashHeavyWeeks} weeks. Max drawdown: ${ctx.currentDrawdownPct.toFixed(1)}%. Give a personalized 2-3 sentence reflection.`;
      break;
  }
